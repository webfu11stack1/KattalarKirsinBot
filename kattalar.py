...
conn = sqlite3.connect('kattalar.db')
cursor = conn.cursor()
...
with sqlite3.connect('kattalar.db') as conn:
    cursor = conn.cursor()
...
TOKEN = "8390361149:AAElGolNEFTxEhU-ugnfnIc-2tEC8Dzbpu4"
bot = Bot(token=TOKEN)
                ...
                InlineQueryResultArticle(
                    ...
                    reply_markup=InlineKeyboardMarkup().add(
                        InlineKeyboardButton(
                            "📽 Kinoni ko'rish",
                            url=f"https://t.me/KattalarKirsinBot?start={result['movie_code']}"
                        )
                    )
                )
            )
...
async def export_db_command(message: types.Message, state: FSMContext):
    ...
    db_file_path = 'kattalar.db'  # Bazangizning yo'li
    ...
    with sqlite3.connect('kattalar.db') as conn:
        ...
        # )
            ...
            if unsubscribed_channels:
                ...
                keyboard.add(InlineKeyboardButton(text="➕ Obuna bo'lish 1", url="https://www.instagram.com/kattalar_kirsin"))
                ...
                keyboard.add(InlineKeyboardButton(text="Tekshirish ✅", url="https://t.me/KattalarKirsinBot?start=True"))
...
async def premium_info(callback_query: types.CallbackQuery,state:FSMContext):
    ...
    buy_button = InlineKeyboardMarkup(row_width=2).add(
        ...
        InlineKeyboardButton("⬅️ Orqaga", url="https://t.me/KattalarKirsinBot?start=True")
    )
    await callback_query.message.edit_text(text, parse_mode="HTML", reply_markup=buy_button)
    await state.set_state("byprem")
...
async def buy_premium(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    await state.set_state("photoprem")



CHANNEL_ID_PRM = -1002958532243  # admin kanal ID
# ADMIN_ID_PRM = 123456789  # admin foydalanuvchi ID (xohlovchi rad/qo‘shish tugmasi bosadi)



@dp.message_handler(content_types=['photo'], state="photoprem")
...
async def cancel_suggestion(callback_query: types.CallbackQuery, state: FSMContext):
    kanalim = InlineKeyboardMarkup(
             inline_keyboard=[
                [InlineKeyboardButton(text="🎥 Top Filmlar Kanali", url="https://t.me/+b0Mi_YBWHecyNGQ6"),
    ...
    try:
        if movie_code:
            ...
            keyboard.add(
                InlineKeyboardButton(
                    text="🎥 Kino ko'rish", 
                    url=f"https://t.me/KattalarKirsinBot?start={movie_code}"
                )
            )
            ...
            botga = InlineKeyboardMarkup(
                inline_keyboard=[
                    [InlineKeyboardButton(text="🔙 Botga o'tish", url="https://t.me/KattalarKirsinBot"),
